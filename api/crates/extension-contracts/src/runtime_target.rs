use crate::error::{ContractResult, ExtensionContractError};
...
impl RuntimeTarget {
    ...
    pub fn from_rust_target_triple(raw: &str) -> ContractResult<Self> {
        match raw.trim() {
    ...
    pub fn from_host_parts(os: &str, arch: &str) -> ContractResult<Self> {
        match (os, arch) {
            ...
            (left_os, left_arch) => Err(ExtensionContractError::invalid_provider_contract(
    ...
    pub fn current_host() -> ContractResult<Self> {
        Self::from_host_parts(std::env::consts::OS, std::env::consts::ARCH)
    }

    pub fn from_executable_bytes(bytes: &[u8]) -> ContractResult<Option<Self>> {
        if !bytes.starts_with(b"\x7fELF") {
            return Ok(None);
        }
        if bytes.len() < 20 {
            return Err(ExtensionContractError::invalid_provider_package(
        ...
        let target = match machine {
            ...
            _ => {
                return Err(ExtensionContractError::invalid_provider_package(format!(
