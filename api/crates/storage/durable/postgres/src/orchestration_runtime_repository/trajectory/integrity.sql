with observations as (
    ...
    select coalesce(o.invocation_id,i.invocation_id) as invocation_id,
    ...
    from observations o full join latest_protocol i using(invocation_id,attempt)
), steps as (
    select invocation_id,provider_attempt_index as attempt,
    ...
    from provider_semantic_trajectory_steps where flow_run_id=$1 and node_run_id=$2
    group by 1,2,3
), native as (
    select coalesce(s.invocation_id,i.invocation_id) as invocation_id,
    ...
    full join (select * from native_trajectory_integrity where flow_run_id=$1 and node_run_id=$2) i
        on s.invocation_id=i.invocation_id and s.attempt=i.provider_attempt_index
), semantic as (
    ...
    from (select * from steps where source<>'ai_native') s
    left join protocol p using(invocation_id,attempt)
    union all
    ...
    where not p.unavailable
        and not exists(select 1 from steps s where s.invocation_id=p.invocation_id and s.attempt=p.attempt)
        and not exists(select 1 from native n where n.invocation_id=p.invocation_id and n.attempt=p.attempt)
)
