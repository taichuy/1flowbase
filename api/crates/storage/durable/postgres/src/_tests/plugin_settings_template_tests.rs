...
async fn root_2014_ac_010_template_trigger_matrix() {
    ...
    let created = store.list_ui_code_templates().await.unwrap().remove(0);
...
async fn root_2014_ac_011_atomic_version_retry_concurrency() {
    ...
    let old_target = store
        ...
        .pop()
        .unwrap();
    let id = store.list_ui_code_templates().await.unwrap()[0].id;
    ...
    assert_eq!(store.list_ui_code_templates().await.unwrap().len(), 2);
}
