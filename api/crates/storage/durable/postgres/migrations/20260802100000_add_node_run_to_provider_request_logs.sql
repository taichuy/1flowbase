-- Request logs may outlive runtime rows, matching the existing unconstrained flow_run_id snapshot.
alter table model_provider_request_logs
    add column if not exists node_run_id uuid;

create index if not exists model_provider_request_logs_scope_flow_run_started_idx
    on model_provider_request_logs (scope_id, flow_run_id, started_at desc, id desc);

create temporary table model_provider_request_log_added_fields (
...
insert into model_provider_request_log_added_fields (
    field_id, code, title, field_kind, is_required, is_unique, sort_order
)
values
    ('00000000-1535-4000-8000-000000000029', 'node_run_id', 'Node run ID', 'many_to_one', false, false, 28);
...
select
    ...
    null,
    null
from model_provider_request_log_added_fields fields
