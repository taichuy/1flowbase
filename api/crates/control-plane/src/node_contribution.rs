...
const SHARED_PROCESSING_NODES: &[BuiltinNodeSpec] = &[
    ready("llm", "LLM", "Calls a configured language model and returns generated text, usage, and optional structured output.", "ai"),
    unavailable("knowledge_retrieval", "Knowledge Retrieval", "Retrieves documents from a knowledge source. The authoring contract is reserved, but runtime execution is not implemented.", "ai"),
    unavailable("question_classifier", "Question Classifier", "Classifies input into configured branches. The authoring contract is reserved, but runtime execution is not implemented.", "logic"),
    ready("if_else", "If / Else", "Evaluates ordered conditions and activates the matching branch.", "logic"),
    ready("code", "Code", "Executes configured isolated code and returns structured outputs.", "transform"),
    ready("template_transform", "Template Transform", "Renders a templated value from upstream variables.", "transform"),
    ready("http_request", "HTTP Request", "Calls an external HTTP endpoint and exposes its response.", "integration"),
    ready("tool", "Tool", "Pauses for an external tool callback and resumes with the tool result.", "integration"),
    ready("tool_result", "Tool Result", "Projects tool callback content into flow outputs.", "integration"),
    ...
    ready("variable_assigner", "Variable Assigner", "Writes configured values into the run or conversation variable pool.", "transform"),
    unavailable("parameter_extractor", "Parameter Extractor", "Extracts structured parameters from text. The authoring contract is reserved, but runtime execution is not implemented.", "ai"),
    unavailable("iteration", "Iteration", "Runs a child graph for collection items. The authoring contract is reserved, but runtime execution is not implemented.", "logic"),
    unavailable("loop", "Loop", "Repeats a child graph while a condition holds. The authoring contract is reserved, but runtime execution is not implemented.", "logic"),
    ready("human_input", "Human Input", "Pauses execution until a user supplies the requested input.", "interaction"),
];
