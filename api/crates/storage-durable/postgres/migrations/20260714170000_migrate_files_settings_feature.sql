...
values (
  md5('permission_definition:settings_feature.access.system.files')::uuid,
...
begin
  ...
  from (
    values
      ('settings_route.visible.settings.files', 'settings_feature.access.system.files'),
      ('settings_route.visible.settings.files', 'file_storage.view.all'),
      ('settings_route.visible.settings.files', 'file_storage.manage.all'),
      ('settings_route.visible.settings.files', 'file_table.view.all'),
      ('settings_route.visible.settings.files', 'file_table.view.own'),
      ('settings_route.visible.settings.files', 'file_table.create.all'),
      ('settings_route.visible.settings.files', 'file_table.delete.all'),
      ('settings_route.visible.settings.files', 'file_table.delete.own'),
      ('settings_route.visible.settings.files', 'file_table.bind.all')
  ...
  if missing_target is not null then
    raise exception 'files SettingsFeature grant migration target is missing: %', missing_target;
  end if;
end;
$$;
...
begin
  if exists (
    ...
    raise exception 'files SettingsFeature grant migration verification failed';
  end if;
end;
$$;
...
where grants.permission_id = definitions.id
  and definitions.code = 'settings_route.visible.settings.files';

delete from permission_definitions
where code = 'settings_route.visible.settings.files';
