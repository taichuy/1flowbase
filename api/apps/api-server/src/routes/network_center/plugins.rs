...
use crate::{
    ...
    routes::{
        ...
        plugins::{
            ...
            OfficialPluginArtifactResponse, OfficialPluginCatalogEntryResponse,
            OfficialPluginCatalogPageResponse, OfficialPluginCatalogQuery,
            OfficialPluginCatalogResponse, PluginUploadMultipartBody,
        },
    },
};
...
#[utoipa::path(
    ...
    responses((status = 200, body = OfficialPluginCatalogResponse), (status = 401, body = crate::error_response::ErrorBody), (status = 403, body = crate::error_response::ErrorBody))
)]
    ...
    let installed = local_catalog
        ...
        .map(|entry| {
            (
                format!(
                    "{}:{}/{}",
                    entry.installation.category.as_str(),
                    entry.installation.organization,
                    entry.installation.provider_code
                ),
                if entry.local_artifact.artifact_status
...
fn project_catalog_entry(
    state: &ApiState,
    entry: crate::official_extension_catalog::OfficialExtensionCatalogEntry,
    installed: &HashMap<String, &'static str>,
