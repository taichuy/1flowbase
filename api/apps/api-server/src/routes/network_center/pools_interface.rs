...
pub(crate) enum NetworkPoolsInput {
    ...
    UpdateProxy {
        provider_id: String,
        body: CreateNetworkEgressProxyBody,
    },
    TestMember {
        pool_id: String,
        member_id: String,
...
pub(crate) enum NetworkPoolsOutput {
    Proxy(NetworkEgressProxyResponse),
...
impl NetworkPoolsAdapter {
    ...
    async fn execute_inner(
        ...
        match input {
            NetworkPoolsInput::GetProxy { provider_id } => {
                Ok(NetworkPoolsOutput::Proxy(proxy_response(
                    self.proxy_service()
                        .get_proxy(parse_uuid(&provider_id, "provider_id")?)
                        .await?,
                )?))
            }
            NetworkPoolsInput::UpdateProxy { provider_id, body } => {
                let provider_id = parse_uuid(&provider_id, "provider_id")?;
                ...
                service
                    ...
                    .await?;
                Ok(NetworkPoolsOutput::Proxy(proxy_response(
                    service.get_proxy(provider_id).await?,
                )?))
            }
...
const DECLARATIONS: &[ConsoleInterfaceDeclaration] = &[
    ConsoleInterfaceDeclaration {
        interface_id: "network_egress_proxies.get",
        binding_id: "http.console.network-egress-proxies.get.v1",
        method: "GET",
        path: "/api/console/network-center/pools/proxies/:provider_id",
        mutating: false,
    },
    ConsoleInterfaceDeclaration {
        interface_id: "network_egress_proxies.update",
        binding_id: "http.console.network-egress-proxies.update.v1",
        method: "PATCH",
        path: "/api/console/network-center/pools/proxies/:provider_id",
        mutating: true,
    },
