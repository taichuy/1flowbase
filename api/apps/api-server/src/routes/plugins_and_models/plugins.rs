...
use control_plane::plugin_management::{
    AssignPluginCommand, DeletePluginFamilyCommand, EnablePluginCommand, ExtensionCatalogCategory,
    InstallCurrentNodePluginArtifactCommand, InstallOfficialExtensionCommand,
    InstallOfficialPluginCommand, InstallPluginCommand, InstallPluginResult,
    InstallUploadedPluginCommand, LocalExtensionInventoryEntry, OfficialPluginCatalogEntry,
...
pub async fn list_local_extension_inventory(
    ...
    let page = extension_center_service(&state, "extension_center.installed.view")
...
async fn load_extension_catalog_gateway_page(
    state: &ApiState,
    actor_user_id: Uuid,
    ...
    if let Some(plugin_type) = category.plugin_type() {
        let view = extension_center_service(state, "extension_center.catalog.view")
            .list_official_catalog(
                actor_user_id,
...
pub async fn list_extension_catalog_gateway(
    ...
    let page = load_extension_catalog_gateway_page(
        &state,
        context.user.id,
        category,
        query.cursor,
...
async fn install_or_update_official_extension(
    ...
    let result = extension_center_service(state, operation_id)
...
pub async fn install_uploaded_extension(
    ...
    let result = extension_center_service(&state, "extension_center.install.upload")
