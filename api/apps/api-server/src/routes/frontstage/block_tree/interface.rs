...
use control_plane::frontstage::{
    FrontstageSourceTextEdit, ReplaceFrontstageBlockCodeCommand, SearchFrontstageBlockCodeCommand,
};
...
pub(crate) enum FrontstageBlocksInput {
    ...
    SearchCode(String, String, SearchFrontstageBlockCodeQuery),
    ReplaceCode(String, String, ReplaceFrontstageBlockCodeBody),
}
...
impl InterfaceContract for FrontstageBlocksInput {
    fn managed_projection_schema() -> Option<serde_json::Value> {
        ...
        Some(mp::union_schema(vec![
            mp::object_schema(&[
                ("variant", mp::tag_schema("SearchCode")),
                ("query_bytes", mp::count_schema()),
            ]),
            mp::object_schema(&[
                ("variant", mp::tag_schema("ReplaceCode")),
                ("edit_count", mp::count_schema()),
            ]),
            mp::object_schema(&[
...
pub(crate) enum FrontstageBlocksOutput {
    ...
    CodeSearch(FrontstageCodeSearchResponse),
    EditedCode(FrontstageCodeEditReceiptResponse),
...
impl InterfaceContract for FrontstageBlocksOutput {
    fn managed_projection_schema() -> Option<serde_json::Value> {
        ...
        Some(mp::union_schema(vec![
            mp::object_schema(&[
                ("variant", mp::tag_schema("CodeSearch")),
                ("match_count", mp::count_schema()),
            ]),
            mp::object_schema(&[
                ("variant", mp::tag_schema("EditedCode")),
                ("edit_count", mp::count_schema()),
            ]),
        ...
        let output = match input {
            ...
            FrontstageBlocksInput::PatchCode(page_id, block_id, body) => {
                ...
                let code = service()
                    ...
                    .await?;
                FrontstageBlocksOutput::EditedCode(code.into())
            }
            FrontstageBlocksInput::SearchCode(page_id, block_id, query) => {
                FrontstageBlocksOutput::CodeSearch(
                    service()
                        .search_block_code(SearchFrontstageBlockCodeCommand {
                            scope: Self::scope(actor, page_id, block_id)?,
                            query: query.query,
                            start_line: query.start_line,
                            start_column: query.start_column,
            ...
            FrontstageBlocksInput::ReplaceCode(page_id, block_id, body) => {
                FrontstageBlocksOutput::EditedCode(
                    service()
                        .with_node_id(self.0.api_node_id.clone())
                        .replace_block_code(ReplaceFrontstageBlockCodeCommand {
                            scope: Self::scope(actor, page_id, block_id)?,
                            expected_source_revision: body.expected_source_revision,
                            edits: body
                                .edits
                                .into_iter()
                                .map(|edit| FrontstageSourceTextEdit {
...
const DECLARATIONS: &[ConsoleInterfaceDeclaration] = &[
    ConsoleInterfaceDeclaration {
        ...
        binding_id: "http.console.frontstage.blocks.code-search.get.v1",
        method: "GET",
        path: "/api/console/frontstage/pages/:page_id/blocks/:block_id/code/search",
        mutating: false,
    },
    ConsoleInterfaceDeclaration {
        interface_id: "frontstage.blocks.code.update",
        binding_id: "http.console.frontstage.blocks.code-replace.post.v1",
        method: "POST",
        path: "/api/console/frontstage/pages/:page_id/blocks/:block_id/code/replace",
        mutating: true,
    },
    ConsoleInterfaceDeclaration {
