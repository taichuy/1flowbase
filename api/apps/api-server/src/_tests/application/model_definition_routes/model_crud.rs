...
async fn model_definition_routes_manage_models_and_fields_without_publish() {
    ...
    let list_main_source_models = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/console/settings/data-models/model-definitions?source_kind=main_source")
                .header("cookie", &cookie)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn model_definition_routes_filter_models_by_title_code_or_table_id() {
    ...
    let filter_response = app
        ...
        .oneshot(
            Request::builder()
                .uri("/api/console/settings/data-models/model-definitions?source_kind=main_source&filter=%7B%22code%22%3A%7B%22%24includes%22%3A%22customer%22%7D%7D")
                .header("cookie", &cookie)
                .body(Body::empty())
                .unwrap(),
        )
...
async fn model_definition_routes_batch_delete_models_from_action_endpoint() {
    ...
    let list_response = app
        .oneshot(
            Request::builder()
                .uri("/api/console/settings/data-models/model-definitions?source_kind=main_source&filter=%7B%22code%22%3A%7B%22%24includes%22%3A%22orders%22%7D%7D")
                .header("cookie", &cookie)
                .body(Body::empty())
                .unwrap(),
        )
