...
async fn plugin_routes_install_upload_accepts_multipart_package() {
    ...
    assert_eq!(
        payload["data"]["installation"]["plugin_id"],
        "openai_compatible@0.2.0"
    );
    assert_eq!(payload["data"]["installation"]["plugin_version"], "0.2.0");
    assert_eq!(
        payload["data"]["installation"]["local_artifact"]["artifact_status"],
        "ready"
    );
}
