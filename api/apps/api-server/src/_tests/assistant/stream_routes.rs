...
#[tokio::test]
async fn ac_001_assistant_message_routes_match_runtime_path_parameters() {
    let app = test_app().await;
    let (cookie, csrf) = login_and_capture_cookie(&app, "root", "change-me").await;
    let application_id = create_published_agent_flow(&app, &cookie, &csrf).await;
    select_assistant_application(&app, &cookie, &csrf, &application_id).await;
    ...
    let created = app
        ...
        .unwrap();
    assert_eq!(created.status(), StatusCode::CREATED);
    let conversation_id = response_json(created).await["data"]["conversation_id"]
        .as_str()
        .expect("new assistant conversation receives a stable conversation_id")
        .to_string();

    for path in [
        format!("/api/console/assistant/conversations/{conversation_id}/messages"),
        format!(
            "/api/console/assistant/legacy-runs/{}/messages",
            uuid::Uuid::now_v7()
        ),
    ] {
        let response = app
            ...
            .oneshot(
                Request::builder()
                    .method("GET")
                    .uri(format!("{path}?application_id={application_id}"))
                    .header("cookie", &cookie)
                    .body(Body::empty())
                    .unwrap(),
            )
