...
#[test]
fn openai_chat_completions_accepts_bearer_and_preserves_model() {
...
#[test]
fn opencode_chat_stream_options_cross_the_request_boundary() {
    run_compat_route_test(|| async {
...
#[test]
fn openai_chat_completions_accepts_root_endpoint_for_plain_base_url_clients() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_chat_completions_rejects_removed_prefixed_openai_alias() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_compatible_routes_reject_nested_v1_aliases() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_responses_accepts_blocking_text_input() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn codex_parallel_tool_calls_false_crosses_the_request_boundary() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_public_runs_persist_trusted_compatibility_mode() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn openai_responses_accepts_root_endpoint_for_plain_base_url_clients() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_responses_resolves_previous_response_id_before_creating_a_run() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
        let token =
            setup_published_app(&app, "OpenAI Responses Unsupported Continuation App").await;
...
#[test]
fn openai_responses_treats_opaque_previous_response_id_as_provider_lookup() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn openai_responses_rejects_missing_previous_response_before_creating_a_run() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn openai_responses_function_call_output_resolves_callback_before_run_creation() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn d4_ac_016_openai_responses_input_file_reaches_the_selected_provider_capability_boundary() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn d2_ac_001_openai_nested_unknown_content_fields_reject_before_run_or_provider() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
...
#[test]
fn openai_models_lists_start_node_configured_models() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn native_models_returns_canonical_start_node_model_capabilities() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_models_with_client_version_returns_codex_model_metadata() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_models_accepts_full_chat_completions_base_url_alias() {
    run_compat_route_test(|| async {
        let app = test_app().await;
...
#[test]
fn openai_chat_accepts_tools_and_creates_a_run() {
    run_compat_route_test(|| async {
        let (app, state) = test_app_with_state().await;
