...
import {
  ...
  writeMcpToolsRouteState,
  type McpToolsRouteState
} from './McpToolsTab/route-state';
...
export function McpToolsTab({
  ...
  const { modalOpen, editingTool, step, exportingCatalog } = toolsState;
  const [routeState, setRouteState] = useState(readMcpToolsRouteState);
  ...
  const updateRouteState = useCallback(
    (nextState: McpToolsRouteState, mode: 'push' | 'replace' = 'push') => {
      writeMcpToolsRouteState(nextState, mode);
  ...
  useEffect(() => {
    const handlePopState = () => {
      const nextState = readMcpToolsRouteState();
      setRouteState(nextState);
  ...
  const parameterSchemaValidRef = useRef(true);
  const resultSchemaValidRef = useRef(true);
  ...
  const setParameterSchemaValue = useCallback(
    (schema: ToolFormValues['parameter_schema']) =>
      form.setFieldValue('parameter_schema', schema),
    [form]
  );
  const setResultSchemaValue = useCallback(
    (schema: ToolFormValues['result_schema']) =>
      form.setFieldValue('result_schema', schema),
    [form]
  );
  ...
  const setParameterSchemaValidity = useCallback((valid: boolean) => {
    parameterSchemaValidRef.current = valid;
  }, []);
  const setResultSchemaValidity = useCallback((valid: boolean) => {
    resultSchemaValidRef.current = valid;
  }, []);
  ...
  const saveToolMutation = useMutation({
    mutationFn: (values: ToolFormValues) => {
      if (!parameterSchemaValidRef.current) {
        throw new Error('parameter_schema JSON');
      }
      if (!resultSchemaValidRef.current) {
  ...
  const filteredTools = catalog.tools.filter((tool) => {
    ...
    return (
      (!routeState.keyword ||
        text.includes(routeState.keyword.toLowerCase())) &&
      (!routeState.execution_target_kind ||
        tool.execution_target.kind === routeState.execution_target_kind) &&
      (!routeState.interface_id ||
        (tool.execution_target.kind === 'interface_wrapper' &&
          tool.execution_target.interface_id === routeState.interface_id)) &&
      (!routeState.risk_level || tool.risk_level === routeState.risk_level) &&
      (!routeState.status || tool.status === routeState.status) &&
      (routeState.des_id_required === undefined ||
        tool.des_id_required === routeState.des_id_required)
    );
  });
  const pageCount = Math.max(
    1,
    Math.ceil(filteredTools.length / MCP_TOOLS_PAGE_SIZE)
  );
  const visibleTools = filteredTools.slice(
    (routeState.page - 1) * MCP_TOOLS_PAGE_SIZE,
    routeState.page * MCP_TOOLS_PAGE_SIZE
  );

  useEffect(() => {
    if (routeState.page <= pageCount) {
      return;
    }
  ...
  const applyFilters = useCallback(() => {
    const nextState: McpToolsRouteState = {
      ...filterDraft,
      page: 1,
    ...
    setFilterDraft(nextState);
  }, [filterDraft, updateRouteState]);

  const resetFilters = useCallback(() => {
    const nextState: McpToolsRouteState = { page: 1 };
    updateRouteState(nextState);
    setFilterDraft(nextState);
  }, [updateRouteState]);
  ...
  return (
    <>
      <DataTableLayout
        filters={
          <DataTableFilterForm
            ariaLabel={i18nText('settingsMcpManagement', 'auto.tool_filters')}
            collapseLabel={i18nText(
              'settingsMcpManagement',
              'auto.collapse_filters'
            )}
            ...
            expandedFields={
              <>
                <DataTableFilterField label="risk_level">
                  <Select<McpToolsRouteState['risk_level']>
                    allowClear
                ...
                <DataTableFilterField label="des_id_required">
                  <Select<'true' | 'false'>
                    allowClear
                    placeholder="des_id_required"
                    value={
                      filterDraft.des_id_required === undefined
                        ? undefined
                        : (String(filterDraft.des_id_required) as
                            | 'true'
                            | 'false')
                    }
                    options={[
                ...
                <DataTableFilterField label="status">
                  <Select<McpToolsRouteState['status']>
                    allowClear
                    placeholder="status"
                    ...
                    onChange={(status) =>
                      setFilterDraft((current) => ({ ...current, status }))
                    }
                  />
                </DataTableFilterField>
              </>
          ...
          >
            <DataTableFilterField
              label={i18nText('settingsMcpManagement', 'auto.tool_keyword')}
            >
              <Input
                allowClear
                placeholder={i18nText(
                  'settingsMcpManagement',
                  'auto.tool_keyword_placeholder'
                )}
                type="search"
            ...
            <DataTableFilterField
              label={i18nText('settingsMcpManagement', 'auto.tool_type')}
            >
              <Select<McpToolsRouteState['execution_target_kind']>
                ...
                value={filterDraft.execution_target_kind}
                options={[
                  {
                    label: i18nText(
                      'settingsMcpManagement',
                ...
                onChange={(execution_target_kind) =>
                  setFilterDraft((current) => ({
                    ...current,
      ...
      >
        <DataTable
          ...
          onPageChange={(page) => updateRouteState({ ...routeState, page })}
        />
      </DataTableLayout>
      <FixedHeightModal
        width={840}
      ...
      >
        ...
        >
          <div hidden={step !== 'basic'}>
            ...
            <Form.Item htmlFor="tool_id" label="tool_id" required>
              {editingTool ? (
                <Form.Item
                  name="tool_id"
                  noStyle
            ...
            <Form.Item htmlFor="des_id" label="des_id" required>
              <Space.Compact block>
                <Form.Item
                  name="des_id"
                  noStyle
            ...
            <div>
              {editingTool?.execution_target.kind === 'mcp_proxy' ? (
                <Space
                  ...
                  className="mcp-management__stack"
                >
                  <Typography.Text strong>parameter_schema</Typography.Text>
                  <Form.Item
                    noStyle
                    shouldUpdate={(previous, current) =>
                      previous.parameter_schema !== current.parameter_schema
                    }
                  >
                    {({ getFieldValue }) => (
                      <JsonSchemaInlineEditor
                        fallbackRootType="object"
                        resetKey={`parameter:${schemaEditorRevision}`}
                        schema={schemaRecord(getFieldValue('parameter_schema'))}
                        structureMode="fields"
                        onChange={setParameterSchemaValue}
                        onValidityChange={setParameterSchemaValidity}
                      />
                    )}
                  </Form.Item>
                  <Typography.Text strong>input_mapping</Typography.Text>
                  <Form.Item
                    noStyle
                    shouldUpdate={(previous, current) =>
            ...
            <div>
              {editingTool?.execution_target.kind === 'mcp_proxy' ? (
                <Space
                  ...
                  className="mcp-management__stack"
                >
                  <Typography.Text strong>result_schema</Typography.Text>
                  <Form.Item
                    noStyle
                    shouldUpdate={(previous, current) =>
                      previous.result_schema !== current.result_schema
                    }
                  >
                    {({ getFieldValue }) => (
                      <JsonSchemaInlineEditor
                        fallbackRootType="object"
                        resetKey={`result:${schemaEditorRevision}`}
                        schema={schemaRecord(getFieldValue('result_schema'))}
                        structureMode="fields"
                        onChange={setResultSchemaValue}
                        onValidityChange={setResultSchemaValidity}
                      />
                    )}
                  </Form.Item>
                  <Typography.Text strong>output_mapping</Typography.Text>
                  <Form.Item
                    noStyle
                    shouldUpdate={(previous, current) =>
