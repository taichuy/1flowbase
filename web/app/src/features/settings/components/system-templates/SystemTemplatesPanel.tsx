...
export function SystemTemplatesPanel() {
  ...
  const readFile = async (file: File) => {
    ...
    try {
      ...
      if (version === readVersion.current) setFileError(true);
    } finally {
  ...
  return (
    <SettingsSectionSurface>
      ...
      <Alert
        type="info"
        showIcon
        title={t('structure_notice')}
        description={t('plugin_notice')}
      />
      <Space wrap>
        <Button
          onClick={() => {
        ...
        >
          <Button
            loading={reading || previewMutation.isPending}
            disabled={busy}
          >
            {t('import')}
          </Button>
      ...
      {importFile && (
        <Space orientation="vertical" size="middle" style={{ width: '100%' }}>
              ...
              {preview.dependencies.length > 0 && (
                <Table
                  ...
                  columns={[
                    ...
                    { title: t('contribution'), dataIndex: 'contribution_code' }
                  ]}
                />
              )}
            </>
          ...
          {result && (
            <>
              <Alert
                ...
                description={!result.complete ? t('partial_notice') : undefined}
              />
          ...
          <Space wrap>
            ...
            <Button disabled={busy} onClick={resetImport}>
              {t('clear')}
            </Button>
          </Space>
        </Space>
      ...
      <Modal
        ...
        title={t('export')}
        onCancel={() => {
          if (!exportMutation.isPending) setExportOpen(false);
        }}
        onOk={() => void download()}
        okText={t('download')}
        cancelText={t('cancel')}
