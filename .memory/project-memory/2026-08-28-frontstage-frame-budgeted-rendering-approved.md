...
summary: Frontstage 帧预算调度与稳定尺寸提交已通过定向 QA 和真实页面用户验收，#1925 已关闭。
keywords:
  - Frontstage
  - Native Block
  - scheduler
...
- Single Issue [#1925](https://github.com/taichuy/1flowbase/issues/1925) 已完成并关闭。
...
- 截止日期：未指定；用户已于 2026-08-28 完成真实页面验收。
...
- 编译原本已运行在 Worker；未命中新增 Worker 的停止条件。用户确认真实页面效果后，#1925 已进入 `phase:closed` 并关闭。
