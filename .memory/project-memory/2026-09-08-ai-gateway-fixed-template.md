...
2026-09-10 11 用户恢复并明确要求修改 Block 01a0746e-53d6-7c01-9ddd-fcf0dffb5c5e，覆盖此前暂缓 GUI 的阶段边界。Codex 已经通过 MCP 保存 Block：模板表单完整配置保留、压缩阈值精度保留/预览、输出上限校验与弹窗内滚动；实际浏览器保存发布目标 1flowbase管家（01a08675-0709-7a73-b4b1-161c636b74c1）第7版，空 model 默认路由及 1000000/850000 上下文/阈值已落地。现有 LLM config、MCP 关联与提示词不变；没有执行真实模型推理。详情以 tmp/test-governance/gateway-block/qa-report.md 和实时配置为准，无截止日期。

2026-09-10 11 用户新生成应用 01a0895c-097c-71f3-abc7-c90873ab9509 预览报 invalid input: bindings。根因为生成 Workflow 将系统初始默认 node-answer 的 node-llm.text 引用合入新网关，而 node-llm 已被替换；之前只验证已有网关保存，漏掉首次生成+真实聊天。Codex 已在原授权范围通过 MCP 修复并发布生成 Workflow 的默认草稿合并规则，单独修复并重新发布用户指定应用回复引用。2项定向回归、真实新生成文档编译/运行、3100完整模型聊天5节点全部成功（回复“你好！很高兴为你服务。”）。产品源码与 Block 未改；证据 tmp/test-governance/gateway-bindings/qa-report.md。以后固定模板变更须覆盖首次生成系统默认草稿和已有网关保留配置两条路径；没有截止日期。
