...
- 当前活动 Issue：[#1900 支持并排行与独占行之间的二维拖拽投影](https://github.com/taichuy/1flowbase/issues/1900)，已于 2026-08-27 完成实现并进入 `phase:user-acceptance`。自动布局 drag session 使用缓存的逻辑行序列与二维 intent classifier，互斥表达 `JoinRow(rowIndex, cellIndex)` 和 `InsertStandaloneRow(rowIndex)`；纵向边界采用进入／退出不同阈值的滞回，行位置由成员最大高度前缀和生成。PageCanvas 提供 content-space pointer，compactor 缓存 pointer-to-active offset 以兼容 #1899 无 pointermove 的边缘滚动；不改变持久化布局 contract 或 RGL patch。
