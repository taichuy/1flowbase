...
- 规则：同时区分运行时调用图与 Cargo 构建依赖图；运行时箭头表示数据/控制流，构建箭头表示 import/编译依赖，基础设施通过 Ports 做依赖倒置时两者不能画成同向。用户认可前期采用 Modular Monolith：一个部署单元内优先 trait/函数调用，不为内部模块引入 HTTP、重试和跨服务一致性。
