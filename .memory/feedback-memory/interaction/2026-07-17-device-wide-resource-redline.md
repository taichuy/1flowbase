...
## Startup diagnosis boundary (2026-09-14)

- 规则：用户反馈 `dev-up` 启动长时间卡住时，先读日志、源码及轻量资源快照，不擅自运行启动命令或重型编译复现。
- 原因：启动可能拖死开发虚拟机，导致用户再次手动终止，显著增加维护成本。
- 适用：本地启动性能诊断和优化建议；真实启动验证需遵守当轮用户明确边界。
