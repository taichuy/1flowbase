...
- `agent-flow` 是远程模板发布版本与本地已安装模板库；扩展中心负责下载、验签和安装记录，应用页面的“导入”只从已安装模板创建新应用。
...
- `extension_installations` 是本地安装状态、版本历史、current 指针、文件路径、checksum 与签名状态的唯一业务真值；文件系统只保存 artifact 字节，不通过目录扫描表达已安装状态。
- 已安装页、模板页和应用模板选择器只查询数据库安装记录，不为判断“已安装”再次请求 GitHub，也不扫描 storage / plugin 目录。
