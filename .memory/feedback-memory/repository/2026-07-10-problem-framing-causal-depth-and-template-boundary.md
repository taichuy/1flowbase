...
summary: 需求对齐要主动指出用户未问但可能更关键的问题；拆解以是否讲清因果机制为标准，不以抽象层级数量为标准。Skill 正文只保留稳定流程和读取条件，完整 issue 格式放入 references 模板。
keywords:
  - problem-framing
  - causal mechanism
  - hidden question
...
用户指出，深度不等于层级多。空洞抽象会让需求看似完整，却没有解释真正的驱动因素，也不能改善方案选择。完整 issue 格式属于按需使用的产物模板，不应挤占每次触发 Skill 都会加载的核心正文。
