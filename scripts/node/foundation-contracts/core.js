...
const FOUNDATION_DEFINITIONS = {
  "ai-gateway": {
    ...
    matches(filePath) {
      return (
        /^scripts\/node\/(?:ai-gateway-concurrency|provider-conformance|verify-state-protocols)(?:\/|\.js$)/u.test(
          filePath,
        ) ||
        /^scripts\/node\/cli\/(?:ai-gateway|acp-claude-smoke)/u.test(
          filePath,
        ) ||
        /^api\/apps\/api-server\/src\/routes\/application_public_api\/(?:anthropic\.rs|compat_sse(?:\/|\.rs$))/u.test(
          filePath,
        ) ||
  ...
  "mcp-gateway": {
    ...
    matches(filePath) {
      return (
        /^api\/(?:apps|crates)\/.*(?:\/|_)(?:mcp)(?:\/|_|\.|-)/iu.test(
          filePath,
        ) ||
        /^scripts\/node\/(?:export-mcp-instance-to-official|mcp-)/u.test(
          filePath,
        )
      );
    },
  ...
  "application-backend": {
    ...
    matches(filePath) {
      return (
        /^api\/crates\/control-plane\/src\/(?:model_definition|_tests\/model_definition)/u.test(
          filePath,
        ) ||
        /^api\/crates\/storage-durable\/postgres\/(?:migrations\/.*model_definition|src\/(?:model_definition_repository|mappers\/model_definition_mapper)|src\/_tests\/model_definition)/u.test(
          filePath,
        ) ||
        /^api\/apps\/api-server\/src\/(?:_tests\/application\/model_definition_routes|routes\/plugins_and_models\/(?:model_definitions|runtime_models|data_sources)|openapi(?:_interface)?\/)/u.test(
          filePath,
        ) ||
        /^api\/crates\/runtime-core\/src\/(?:runtime_model_registry|runtime_record_repository|runtime_acl|model_metadata|_tests\/(?:runtime_model_registry_tests|runtime_acl_tests))\.rs$/u.test(
          filePath,
        )
      );
    },
  ...
  "native-react": {
    ...
    matches(filePath) {
      ...
      return (
        /^web\/packages\/(?:page-runtime|block-sdk)\/src\/.*\.(?:ts|tsx)$/u.test(
          filePath,
        ) ||
        /^web\/app\/src\/(?:features\/frontstage|shared\/code-block)\/.*\.(?:ts|tsx)$/u.test(
          filePath,
        ) ||
    ...
    fast: [
      {
        ...
        args: [
          ...
          "test",
          "--",
          "src/_tests/native-react-compiler",
          "src/_tests/native-trusted-block",
        ],
        cwd: ".",
      },
      {
        ...
        args: [
          ...
          "test",
          "--",
          "src/_tests/native-react-contract.test.ts",
        ],
        cwd: ".",
      },
      {
...
function normalizeChangedFiles(changedFiles) {
  return [
    ...new Set(
      changedFiles
        .map((filePath) => filePath.replace(/\\/gu, "/").trim())
        .filter(Boolean),
    ),
...
function validatePackInventory(overrides = {}) {
  ...
  if (operations.join(" -> ") !== MCP_CORE_OPERATIONS.join(" -> ")) {
    throw new Error(
      `MCP core operations must remain ${MCP_CORE_OPERATIONS.join(" -> ")}`,
    );
  }
  ...
  for (const foundation of FOUNDATION_IDS) {
    ...
    if (
      !definition ||
      definition.fast.length === 0 ||
      definition.full.length === 0
    ) {
      throw new Error(
        `${foundation} must define finite fast and full evidence packs`,
      );
    }
...
function shouldSelectAllFoundations(changedFiles) {
  return changedFiles.some((filePath) =>
    GOVERNANCE_PATHS.some((pattern) => pattern.test(filePath)),
  );
}

function buildMcpFastPack(changedFiles) {
  const fast = FOUNDATION_DEFINITIONS["mcp-gateway"].fast.map((item) => ({
    ...item,
  }));
  if (
    changedFiles.some((filePath) =>
      /(?:mcp_result|result_delivery|result_receipt)/iu.test(filePath),
    )
  ) {
    fast.push({
      ...
      args: [
        ...
        "-p",
        "storage-postgres",
        "mcp_result_receipt_repository_tests",
      ],
      cwd: "api",
    });
  }
...
function buildFoundationPlan({
  ...
  const selectedFoundations =
    foundation === "all"
      ...
      : FOUNDATION_IDS.filter((foundationId) => {
          ...
          return normalizedFiles.some((filePath) =>
            FOUNDATION_DEFINITIONS[foundationId].matches(filePath),
          );
        });
    ...
    FOUNDATION_IDS.map((foundationId) => {
      ...
      return [
        foundationId,
        {
          ...
          fast:
            foundationId === "mcp-gateway"
              ? buildMcpFastPack(normalizedFiles)
              : definition.fast.map((item) => ({ ...item })),
  ...
  if (
    selectedFoundations.includes("ai-gateway") &&
    normalizedFiles.some((filePath) =>
      /agent[_-]?flow|publish/iu.test(filePath),
    )
  ) {
    seams.push({
...
function buildContractReceipt({
  ...
  const resultByFoundation = new Map(
    componentResults.map((result) => [result.foundation, result]),
  );
  const foundations = plan.selectedFoundations.map((foundation) => {
    ...
    if (!result.candidateSha || result.candidateSha !== candidateSha.trim()) {
      return {
        ...
        errors: [
          `candidate SHA mismatch: expected ${candidateSha.trim()}, received ${result.candidateSha || "missing"}`,
        ],
        uncovered: result.uncovered || [],
        deferredEvidence:
          result.deferredEvidence || plan.packs[foundation].full,
      };
    }
    return {
      foundation,
      ...
      executedPack:
        result.executedPack ||
        plan.packs[foundation].fast.map((item) => item.id),
      warnings: result.warnings || [],
      warningFiles: result.warningFiles || [],
  ...
  const warningFiles = [
    ...new Set(foundations.flatMap((item) => item.warningFiles)),
  ];
  const deferredEvidence = foundations.flatMap((item) =>
    item.deferredEvidence.map((evidence) => `${item.foundation}: ${evidence}`),
  );
  const status = foundations.some(
    (item) =>
      item.status !== "passed" || item.exitCode !== 0 || item.errors.length > 0,
  )
    ? "failed"
    : "passed";
...
function writeQualityGateComponentArtifacts(repoRoot, receipt) {
  ...
  const reportPath = writeJson(
    repoRoot,
    path.join(OUTPUT_ROOT, "quality-gate-report.json"),
    report,
  );
  ...
  fs.writeFileSync(
    ...
    [
      ...
      ...report.foundationContractWarnings.map(
...
function appendGitHubPlanOutputs(plan, outputPath) {
  ...
  const lines = FOUNDATION_IDS.map(
    (foundation) =>
...
function runFastPack({
  ...
  const errors = commandResults
    ...
    .map(
      (item) =>
        `${item.id} exited with ${item.exitCode}${item.error ? `: ${item.error}` : ""}`,
    );
...
async function main(argv = [], deps = {}) {
  ...
  if (options.command === "plan") {
    ...
    const planPath = writeJson(
      repoRoot,
      path.join(OUTPUT_ROOT, "foundation-contract-plan.json"),
      {
        ...plan,
    ...
    appendGitHubPlanOutputs(
      plan,
      deps.githubOutputPath || process.env.GITHUB_OUTPUT,
    );
    writeStdout(
      `[foundation-contracts] planned ${plan.selectedFoundations.join(", ") || "no foundations"}: ${planPath}\n`,
    );
    return 0;
  }
  ...
  if (options.command === "run-fast") {
    const plan = options.plan
      ? JSON.parse(
          fs.readFileSync(path.resolve(repoRoot, options.plan), "utf8"),
        )
      : buildFoundationPlan({
          changedFiles: [],
  ...
  if (options.command === "aggregate") {
    const planPath = path.resolve(
      repoRoot,
      options.plan || path.join(OUTPUT_ROOT, "foundation-contract-plan.json"),
    );
    const plan = JSON.parse(fs.readFileSync(planPath, "utf8"));
    const componentRoot = path.resolve(
      repoRoot,
      options["component-root"] || path.join(OUTPUT_ROOT, "components"),
    );
    ...
    const receiptPath = writeJson(
      repoRoot,
      path.join(OUTPUT_ROOT, "foundation-contract-receipt.json"),
      receipt,
    );
