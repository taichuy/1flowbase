...
test("AC-001/006 routes four foundations and ignores legal non-contract changes", () => {
  const matrix = [
    ...
    [
      "application-backend",
      "api/apps/api-server/src/_tests/application/model_definition_routes/model_crud.rs",
    ],
    [
      "native-react",
  ...
  for (const nativeReactHostContractFile of [
    ...
    const plan = buildFoundationPlan({
      changedFiles: [nativeReactHostContractFile],
    });
  ...
  assert.equal(
    providerSettingsPlan.selectedFoundations.includes("application-backend"),
    false,
  );
});
...
test("AC-002 keeps mcp_result outside the core operations and only adds continuation evidence on risk", () => {
  ...
  assert.throws(
    () =>
      validatePackInventory({
        mcpCoreOperations: ["mcp_list", "mcp_get", "mcp_result", "mcp_call"],
      }),
  ...
  assert.equal(
    corePlan.packs["mcp-gateway"].fast.some(
      (item) => item.id === "mcp-result-continuation",
    ),
    false,
  );
  ...
  const continuationPlan = buildFoundationPlan({
    changedFiles: [
      "api/apps/api-server/src/routes/mcp_protocol/result_delivery.rs",
    ],
  });
  assert.equal(
    continuationPlan.packs["mcp-gateway"].fast.some(
      (item) => item.id === "mcp-result-continuation",
    ),
    true,
  );
...
test("AC-007/009 receipt requires candidate identity and warnings stay advisory", () => {
  ...
  assert.deepEqual(warningReceipt.warningFiles, [
    "tmp/test-governance/native-react.warnings.log",
  ]);
  assert.deepEqual(warningReceipt.warnings, [
    "nightly browser matrix deferred",
  ]);
...
test("AC-001/009 foundation receipt adapts into the unified quality gate component", () => {
  ...
  assert.deepEqual(report.warningFiles, [
    "tmp/test-governance/foundation.warnings.log",
  ]);
  assert.deepEqual(report.foundationContractWarnings, [
    "review deferred browser evidence",
  ]);
  assert.deepEqual(report.deferredEvidence, [
    "native-react: nightly browser matrix",
  ]);

  const repoRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), "foundation-quality-component-"),
  );
  ...
  assert.equal(
    JSON.parse(fs.readFileSync(artifacts.reportPath, "utf8")).scope,
