...
const {
  main: runCheckStyleBoundary,
} = require("../check-style-boundary/core.js");
const { main: runCheckRustBackend } = require("../check-rust-backend/core.js");
const { main: runGateRouter } = require("../gate-router/core.js");
const {
  main: runFoundationContracts,
} = require("../foundation-contracts/core.js");
const {
  main: runGrowthTableReport,
...
const {
  main: runLogQueryContractReport,
...
function usageRuntimeGate(writeStdout = (text) => process.stdout.write(text)) {
  writeStdout(
    "Usage: node scripts/node/cli/runtime-gate.js <page-debug args>\n",
  );
}
...
function runRuntimeGate(argv = [], deps = {}) {
  ...
  return runCommandSequence({
    ...
    scope: "runtime-gate",
    commands: [
      buildRuntimeGateCommand({ argv, repoRoot, env: deps.env || process.env }),
    ],
...
async function main(argv = [], deps = {}) {
  ...
  if (options.command === "check-style-boundary") {
    return (deps.runCheckStyleBoundaryImpl || runCheckStyleBoundary)(
      options.rest,
    );
  }
  ...
  if (options.command === "capacity-report") {
    return (deps.runCapacityReportImpl || runCapacityReport)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "check-rust-backend") {
    return (deps.runCheckRustBackendImpl || runCheckRustBackend)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "foundation-contracts") {
    return (deps.runFoundationContractsImpl || runFoundationContracts)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "growth-table-report") {
    return (deps.runGrowthTableReportImpl || runGrowthTableReport)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "log-query-contract-report") {
    return (deps.runLogQueryContractReportImpl || runLogQueryContractReport)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "raw-jsonb-report") {
    return (deps.runRawJsonbReportImpl || runRawJsonbReport)(
      options.rest,
      deps,
    );
  }
  ...
  if (options.command === "vite-lazy-deps-gate") {
    return (deps.runViteLazyDepsGateImpl || runViteLazyDepsGate)(
      options.rest,
      deps,
    );
  }
