...
方向
- 保守 / Conservative：只在目标列表增加可选排序。
- 平衡 / Balanced：把更新时间作为目标列表默认排序。
- 激进 / Aggressive：抽成通用列表排序能力，供同类列表复用。
...
最终建议
- 建议选保守方向：只在目标列表增加可选排序，不改默认排序。需要你确认作用页面和是否改变默认行为。
```
...
方向
- 保守 / Conservative：只收敛当前漂移点，不碰历史数据和 migration。
- 平衡 / Balanced：先建立 domain matrix，再统一 source of truth 和 API contract。
- 激进 / Aggressive：重做 defaults 体系并设计历史数据迁移。
