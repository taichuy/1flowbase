...
命中后台注册设置项、Settings API、角色授权、HostExtension console contribution、注册 CLI、compiled inventory 或旧 `settings_route.visible.*` 迁移时使用。
...
| Migration | 每个历史角色的有效设置访问变化可预览、可审计；未批准 delta 为零 | 只比较 permission row 数量或只跑 migration 成功 |
...
- 旧 `console-route-registry-hygiene` 在只解析源码时只能作为迁移辅助，不能单独结算 registry ownership。
- Dev Acceptance Gate 跑最小 registry、授权和 CLI fixture；workspace cargo、全量 migration rehearsal 与全仓 hygiene 默认交 CI / beta。
- 无法取得 compiled inventory、鉴权反例或 migration delta 时写 `未验证，不下确定结论`；QA 不自动补授权、改映射或执行语义级修复。
